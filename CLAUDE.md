...
**👉 NASTĘPNE ZADANIE: remote access Faza 3 — telemetria/stany nodów + provisioning** (Faza 1 pompa ✅, Faza 2 CRUD reguł + SQLite ✅ 19.06)
...
- **🔜 Faza 3**: tabela stanów nodów (node, state, timestamp odczytu) z `NODE_TELEMETRY`; dla wybranych nodów (np. solar controller — ~10 param/2min) osobna archiwizacja → Energy Gain / Pump Runtime (agregacje SQL); **provisioning nodów** (podłączenie typu nodu tworzy struktury/bazę/procedury). Potem: serve jako prawdziwy unit systemd (zamiast `-test serve`), `chown bramka` bazy.

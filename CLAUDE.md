...
**👉 remote access Faza 3: dane z nodów do DB + provisioning.** **STAN 22.06: provisioning Faza 1 kroki 1-5 DONE I ZWERYFIKOWANE E2E (JOIN uplink → approve API → JOIN_ACCEPT → node przełącza adres, telemetria 243→16) + bramka 0x00 (izolacja gen1/gen2, gen1 włączona) + persystencja adresu noda w NVS (driver TI, bez syscfg; ZWERYFIKOWANE: power-cycle → node sam zaczytał 0x10). NASTĘPNE: apka Android pod provisioning (endpointy `listjoins`/`approvejoin` gotowe po stronie Go).**

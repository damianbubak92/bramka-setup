...
**👉 remote access Faza 3: dane z nodów do DB + provisioning.** **STAN 22.06: provisioning Faza 1 kroki 1-4 DONE I ZWERYFIKOWANE (JOIN uplink + approve API + JOIN_ACCEPT w dół) + bramka na adresie 0x00 (izolacja gen1/gen2, gen1 może zostać włączona). NASTĘPNE: krok 5 (node parsuje JOIN_ACCEPT → NVS → zmiana adresu), potem apka Android pod provisioning.**
...
- **Następne (krok 5, strona noda CC1310)**: parsowanie `JOIN_ACCEPT` w `rfEchoTx.c` (match `joinAcceptData.factory_id` == FCFG own) → zapis `assigned_addr` w NVS → `NODE_ADDRESS` z hardcode 0xF3 na zmienną z NVS (przy starcie: jest → używa i nie śle JOIN; brak → 0xFF czeka na button) → node raportuje pod nowym adresem (`telemetry node 16`) = pętla provisioningu domknięta. Potem: modyfikacja apki Android pod provisioning (lista pending + approve+nazwa).

...
- **Obserwacja (do sprawdzenia)**: przy reboocie systemd pokazywał spinner „A stop job is running for RPMsg..." — normalny progress shutdownu (`Type=notify`, `TimeoutStopSec=15s`), `???` to mielenie znaków `<->` na konsoli szeregowej. DO WERYFIKACJI z `journalctl -b -1 -u rpmsg-service`: czy proces wyszedł szybko na SIGTERM, czy wisiał 15s → SIGKILL. Jeśli wisiał: fix = `os.Exit(0)` po napisaniu triggera zamiast `select{}` (peer-dead watcher w main.go).

...
**👉 NASTĘPNE ZADANIE (jutro): 1) PRAWDZIWE przerwania GPIO na M4F, 2) remote access.** (kadencja+bucketing ZROBIONE 18.06, SPI/CC1310 link ŻYWY 19.06)
- **🔜 NAJPIERW: prawdziwe GPIO-IRQ na M4F (zamiast pollowania SLAVE_READY)** — decyzja usera. Teraz SLAVE_READY jest POLLowany (2ms), bo `Sciclient_rmIrqSetRaw` (routing MCU_GPIO przez WKUP_MCU_GPIOMUX_INTROUTER0 do NVIC M4F) zwraca **r=-1: DMSC nie przyznaje M4F zasobu introutera obok Linuksa** (RM/board-config). Zadanie: doprowadzić do działającego IRQ na SLAVE_READY (D4/MCU_GPIO0). Kierunki do zbadania: (a) **board-config RM sysfw/DM** — dodać zakres wyjść `WKUP_MCU_GPIOMUX_INTROUTER0` do hosta M4F (`sciclient_defaultBoardCfg_rm`/k3-r5 SBL config; sprawdzić czy edytowalne na tym obrazie Arago, czy Linux/DM jest właścicielem); (b) inny numer OUTP introutera który JEST przyznany; (c) czy Linux można poprosić o zwolnienie/grant przez device-tree. Ref: SDK `examples/drivers/gpio/gpio_input_interrupt` (board.c — `Sciclient_rmIrqSetRaw`, OUTP_6), [[am62-mcu-pin-traps]]. Po sukcesie: w `spi_master.c` wrócić z pollu na IRQ (HwiP + bank ISR jak w board.c) — kod ISR już był napisany (zarchiwizowany w historii git/Session Log 19.06). To usuwa 2ms latencję + obciążenie pollu, zbliża do gen1.
- **✅ Kadencja + bucketing + wyrównany tick + dedup (18.06)** — level-trigger + sprzężenie zwrotne (NIE edge), TIME tick `:00`, dane event-driven. Szczegóły: [[engine-eval-cadence]], Session Log 18.06.
- **✅ SPI/CC1310 most dwukierunkowy + E2E z nodem RF (19.06)** — M4F master ↔ CC1310 slave, ramka 128B `spi_frame.h`, handshake E5(MASTER_READY)/D4(SLAVE_READY) na wolnych padach MCAN1, SLAVE_READY POLLowany (M4F GPIO-IRQ RM-denied obok Linuksa). M4F→node (z ACK noda) i node→M4F→Linux (NODE_TELEMETRY) potwierdzone. Pełne szczegóły + lekcje (A6/B6=UART-bridge!, livelock pollingu, transferCancel, won't-stop): Session Log 19.06, [[am62-mcu-pin-traps]]. STUB `gNodeInQueue` ZASILANY realnymi danymi → `COND_PARAMETER`/`DELTA` odblokowane.

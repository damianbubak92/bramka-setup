...
**👉 JUTRO — remote access Faza 3: dane z nodów do DB + provisioning.** Kolejność (zacznij od #1, najmniejszy krok, odblokowuje resztę):
1. **Telemetria/stany nodów → SQLite.** `NODE_TELEMETRY` (0x40) już DOCHODZI do serve (drain eventów tylko loguje). Teraz: dekoder `MessageStruct`→Go (cgo, wzór `nodecmd.go`) + tabela stanów (node id/type, pola stanu, `timestamp` odczytu). Apka/silnik czyta aktualny stan. DECYZJE: schemat (kolumny per-typ vs generyczne `node/param/value`), gdzie dekodować (serve drain goroutine).
2. **Archiwizacja solar (time-series).** Solar controller ~10 param/2min → tabela `samples` (rośnie!) → agregacje SQL: **Energy Gain** (dzienny/mies./roczny), **Pump Runtime**. Retencja/rollup (raw → dzienne agregaty → kasowanie starych raw), batch insert (wear flash).
3. **Provisioning nodów.** Podłączenie typu nodu tworzy struktury (osobna baza/tabele + procedury obróbki dla solar). Tu **wraca guard `sBuforTemp`** gdy bufor raportuje ([[engine-eval-cadence]]).
4. **Sprzątanie.** `serve` jako prawdziwy unit systemd (zamiast `-test hello`/`-test serve`) + `chown bramka:bramka /var/lib/bramka/bramka.db*` + `systemctl reset-failed rpmsg-service`.

**Drobne TODO (z drogi):** SPI scenario-A RX-routing edge-case + `pending`/drenaż serii + wyciszenie logów SPI; apka gen2 (`sendRawPost` przez pickUrl pod LAN setrules); systemowy `date` na bramce → `git pull && sudo ./setup.sh` (moduł 09).

...
**👉 remote access Faza 3: dane z nodów do DB + provisioning.** **STAN 22.06: provisioning Faza 1 + remove + apka DONE I ZWERYFIKOWANE E2E.** JOIN uplink → approve (apka) → JOIN_ACCEPT → node przełącza adres + NVS (przeżywa reboot) → telemetria; **graceful remove** (potwierdzenie noda → 0xFF → zwolnienie adresu, reużycie); bramka na 0x00 (izolacja gen1/gen2, gen1 włączona). Apka: ekran „Zarządzaj urządzeniami" (lista `listnodes` + status, dodaj/usuń). **NASTĘPNE: push/event-driven UI w apce** (zamiast pollingu: gdy node klika JOIN → samo wyskakuje okienko dodawania; statusy live) — patrz [[app-push-events-todo]].

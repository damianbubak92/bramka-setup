...
**👉 remote access Faza 3: dane z nodów do DB + provisioning.** **STAN 22.06: provisioning Faza 1 kroki 1-5 DONE I ZWERYFIKOWANE E2E (JOIN uplink → approve API → JOIN_ACCEPT → node przełącza adres, telemetria 243→16) + bramka 0x00 (izolacja gen1/gen2, gen1 włączona) + persystencja adresu w flashu noda (driverlib, bez syscfg; przeżywa reboot). NASTĘPNE: apka Android pod provisioning (endpointy `listjoins`/`approvejoin` gotowe po stronie Go).**
...
**Co zostało w Fazie 3:**
2. **Agregacje + retencja solar.** Z `solar_history`: zapytania **Energy Gain** (dzienny/mies./roczny — `energy_gain` to już dzienny total w ostatnim rekordzie doby) + **Pump Runtime**. Retencja/rollup (raw → dzienne agregaty → kasowanie starych raw), batch insert (wear flash). Rdzeń (akumulacja+VIEW) już jest.
3. **Provisioning nodów.** Podłączenie typu nodu tworzy struktury (`param_def` + ew. dedykowana tabela jak `solar_history`). Tu **wraca guard `sBuforTemp`** gdy bufor raportuje ([[engine-eval-cadence]]).
4. **Sprzątanie.** `serve` jako prawdziwy unit systemd (zamiast `-test hello`/`-test serve`) + `chown bramka:bramka /var/lib/bramka/bramka.db*` + `systemctl reset-failed rpmsg-service`. Stara martwa tabela `sample` w dev-bazie (nieszkodliwa, można `DROP`).
...
- **Persystencja NVS — BEZ syscfg (CC1310 go nie ma) = bezpośredni flash driverlib**: rezerwujemy 1 sektor sektoro-wyrównaną `static const uint8_t gIdentityFlash[4096] __attribute__((aligned(4096)))` (linker alokuje w flashu i trzyma kod/rodata z dala). `identity_init` czyta `IdentityRec{magic,addr}` z bazy sektora (waliduje magic + addr w puli) → ustawia `gNodeAddress`. `identity_persist` = `HwiP_disable` → `FlashSectorErase` → `FlashProgram` → `HwiP_restore` (maskujemy IRQ bo erase/program zawiesza odczyt flasha; operacja rzadka, one-shot przy JOIN_ACCEPT). Przeżywa **reboot**; reflash firmware resetuje (sektor wraca do zer = unprovisioned). Includes: `driverlib/flash.h` + `ti/drivers/dpl/HwiP.h`.

...
#include "ti_radio_config.h"   /* SysConfig RadioConfig (custom868), Legacy symbols: RF_cmdProp* */
#include "RFQueue.h"
...
/* JOIN trigger — wywoływane z warstwy aplikacji po 4 s przytrzymania touch pada
 * (patrz gpiointerrupt.c). Ogłasza chip bramce (src 0xFF, factory_id w payloadzie);
 * bramka pokazuje go do zatwierdzenia, potem odsyła JOIN_ACCEPT z przydzielonym adresem. */
void rf_send_join(void)
{
    ...
    joinMsg.type = LIGHT_CONTROLLER;     /* node akcji: przekaźnik światła */
...
void radioTaskInit()
{
    ...
    /* JOIN nie jest przypięty do fizycznego przycisku — wyzwala go warstwa aplikacji
     * (rf_send_join) po 4 s przytrzymania touch pada. */

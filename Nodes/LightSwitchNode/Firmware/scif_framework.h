...
typedef struct {
    ...
    uint16_t bvActiveEvent;       ///< Bit-vector indicating index of the currently active event trigger
    ...
    uint16_t progwucfgManTrigReg; ///< Address of PROGWUnCFG register to be triggered manually by the next EventManTriggerVector
...
typedef struct {
    ...
    /// Base event trigger index for each task, two bits per task
    uint16_t                    taskBaseEvTrigIndexLut;
    /// Sensor Controller clock frequency in active mode (in MHz)
    uint16_t                    activeModeSceClkFreqMhz;

    /// Pointer to the driver setup-specific hardware initialization function
    SCIF_VFPTR                  fptrDriverSetupInit;
    /// Pointer to the driver setup-specific hardware uninitialization function
    SCIF_VFPTR                  fptrDriverSetupUninit;
...
/// I/O pin mode: Peripheral input, inactive
#define AUXIOMODE_PERIPH_INPUT_IDLE             0x00000101
/// I/O pin mode: Event observation output
#define AUXIOMODE_PERIPH_OUTPUT_EV_OBS          0x00000100
/// I/O pin mode: SPIM SCLK output
#define AUXIOMODE_PERIPH_OUTPUT_SPIM_SCLK       0x01000100
/// I/O pin mode: SPIM MOSI output
#define AUXIOMODE_PERIPH_OUTPUT_SPIM_MOSI       0x02000100
/// I/O pin mode: TIMER2 event 0 output
#define AUXIOMODE_PERIPH_OUTPUT_TIMER2_EV0      0x03000100
/// I/O pin mode: TIMER2 event 0 output
#define AUXIOMODE_PERIPH_OUTPUT_TIMER2_EV1      0x04000100
/// I/O pin mode: TIMER2 event 0 output
#define AUXIOMODE_PERIPH_OUTPUT_TIMER2_EV2      0x05000100
/// I/O pin mode: TIMER2 event 0 output
#define AUXIOMODE_PERIPH_OUTPUT_TIMER2_EV3      0x06000100
/// I/O pin mode: TIMER2 event 0 output
#define AUXIOMODE_PERIPH_OUTPUT_TIMER2_PULSE    0x07000100

...
/** \brief Initializes a single I/O pin for Sensor Controller usage
  ...
  *         - \ref AUXIOMODE_PERIPH_INPUT_IDLE
...
/** \brief Common function for manually starting, executing and terminating tasks
  ...
  *     completed) or \ref SCIF_ILLEGAL_OPERATION (attempted to execute an already active task, or
  *     missing \ref scifResetTaskStructs() call). The function call has no effect if unsuccessful.
  */
...
/** \brief Starts the specified tasks
  ...
  * \return
  *     \ref SCIF_SUCCESS if successful, otherwise \ref SCIF_NOT_READY (last non-blocking call has not
  *     completed) or \ref SCIF_ILLEGAL_OPERATION (attempted to start an already active task, or
  *     missing \ref scifResetTaskStructs() call). The function call has no effect if unsuccessful.
  */
